# ============================================================
# app.py — AQI Prediction Streamlit App
# ============================================================
...
def draw_aqi_gauge(aqi_value):
    ...
    angle = np.pi * (1 - aqi_clamped / 500)  # map AQI to angle
...
if predict_btn:
    ...
    # Apply same imputer (for consistency, even though no missing values here)
    input_imputed = imputer.transform(input_data)

    # Predict
    aqi_pred = model.predict(input_imputed)[0]
    aqi_pred = max(0, aqi_pred)  # AQI cannot be negative
    ...
    scale_data = {
        ...
        "Health Impact": [
            "Minimal impact",
            "Minor breathing for sensitive people",
            "Breathing discomfort on prolonged exposure",
            "Breathing discomfort for most",
    ...
    pollutants = {
        ...
        'NOx': nox, 'NH3': nh3, 'SO2': so2, 'O3': o3,
        'CO×10': co * 10  # scale CO for visibility
    }
    fig2, ax2 = plt.subplots(figsize=(10, 4))
    ax2.bar(pollutants.keys(), pollutants.values(), color='steelblue', edgecolor='black', alpha=0.85)
    ...
    # Show AQI scale even before prediction
    st.subheader("📋 AQI Scale Reference")
    scale_data = {
        "Category":     ["Good", "Satisfactory", "Moderate", "Poor", "Very Poor", "Severe"],
        "AQI Range":    ["0–50", "51–100", "101–200", "201–300", "301–400", "401–500"],
...
# ── Footer ─────────────────────────────────────────────────────
st.divider()
st.caption("Built with Scikit-learn & Streamlit · Dataset: Air Quality Data in India (Kaggle)")